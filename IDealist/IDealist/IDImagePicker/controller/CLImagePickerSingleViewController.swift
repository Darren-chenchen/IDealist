...
class CLImagePickerSingleViewController: CLBaseImagePickerViewController {
    ...
    // 单选状态的类型
    var singleType: IDImagePickerType?
    ...
    override func viewDidLoad() {
        ...
        self.bottomViewHYS.constant = UIDevice.id_isX() == true ? 34+44:44
    ...
    @IBAction func clickLookBtn(_ sender: Any) {
        ...
        previewVC.doneClouse = {[weak self] () in
            self?.clickSureBtn(self?.sureBtn ?? UIButton())
        }
...
extension CLImagePickerSingleViewController: UICollectionViewDelegate,UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if self.singleType == nil {  // 说明不是单选
            ...
            cell.imagePickerChooseImage = {[weak self] (needRefresh) in
                self?.setSureBtn()
                if needRefresh {
                    // 判断存在越界的indexpath
                    var arr: [IndexPath] = []
