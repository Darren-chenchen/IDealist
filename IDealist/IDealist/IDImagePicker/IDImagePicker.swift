...
public enum IDImagePickerType {
    case singlePicture   // 图片单选
    case singlePictureCrop   // 单选并裁剪
}
public enum IDImagePickerStatusBarType {
    case black   // 黑色
    case white   // 白色
}

public typealias IDImagePickerClouse = (Array<PHAsset>,UIImage?)->()

public class IDImagePicker: NSObject,UIImagePickerControllerDelegate,UINavigationControllerDelegate {
        
    @objc var cameraPicker: UIImagePickerController!
    
    private weak var superVC: UIViewController?
    
    public var idImagePickerClouse: IDImagePickerClouse?
    
    // 是否隐藏视频文件，默认隐藏
    @objc public var isHiddenVideo: Bool = true
    // 是否隐藏图片文件，显示视频文件，默认不隐藏
    @objc public var isHiddenImage: Bool = false
    // 设置单选图片，单选图片并裁剪属性，默认多选
    public var singleImageChooseType: IDImagePickerType?
    // 设置相机在外部，默认不在外部
    ...
    // 配置导航栏的颜色，默认是毛玻璃效果
    public var navColor: UIColor? = UIColor(white: 0, alpha: 0.8)
    // 配置导航栏文字的颜色
    public var navTitleColor: UIColor? = UIColor.white
    // 配置状态栏的颜色
    public var statusBarType: IDImagePickerStatusBarType = .white
    // 相册中是否展示拍照图片
    ...
    @objc public func id_setupImagePickerWith(maxImagesCount: Int,superVC:UIViewController? = nil,didChooseImageSuccess:@escaping (Array<PHAsset>,UIImage?)->()) {
        ...
        self.idImagePickerClouse = didChooseImageSuccess
        ...
        CLPickersTools.instence.authorize(authorizeClouse: { (state) in
            if state == .authorized {
                let photo = CLImagePickersViewController.share.initWith(
                    ...
                    if self.idImagePickerClouse != nil {
    ...
    @objc public static func convertAssetArrToOriginImage(assetArr:Array<PHAsset>,scale:CGFloat,successClouse:@escaping (UIImage,PHAsset)->(),failedClouse:@escaping ()->()) {
        ...
        for item in assetArr {
            ...
            IDImagePicker.getAssetOrigin(asset: item, dealImageSuccess: { (img, info) in
                if img != nil {
                    // 对图片压缩
...
let info = convertFromUIImagePickerControllerInfoKeyDictionary(info)
        ...
        if type == "public.image" {
            CLPickersTools.instence.authorizeSave { (state) in
                if state == .authorized {
                    ...
                    UIImageWriteToSavedPhotosAlbum(photo as! UIImage, self, #selector(IDImagePicker.image(_:didFinishSavingWithError:contextInfo:)), nil)
                } else {
                    DispatchQueue.main.async(execute: {
            ...
            CLPickersTools.instence.getAssetOrigin(asset: phasset) { (img, info) in
                ...
                if img != nil {
                    ...
                    if self.singleImageChooseType == .singlePictureCrop && self.singleModelImageCanEditor != true {  // 单选
                        ...
                        if self.idImagePickerClouse != nil {
                            self.idImagePickerClouse!([phasset],image)
                        }
                    }
    ...
    func crop(image: UIImage,phasset: PHAsset) {
        ...
        cutVC.clCropClouse = {[weak self] (cutImg) in
            if self?.idImagePickerClouse != nil {
                self?.idImagePickerClouse!([phasset],cutImg)
            }
    ...
    func editor(image: UIImage,phasset: PHAsset) {
        ...
        editorVC.editorImageComplete = {[weak self](img) in
            ...
            if self?.idImagePickerClouse != nil {
                self?.idImagePickerClouse!([phasset],img)
            }
        }
